...
public interface SessionNoteRepository extends JpaRepository<SessionNote, Long> {
    ...
    /** Count AT_RISK / CRITICAL students for a mentor using parameterized statuses. */
    @Query("SELECT COUNT(DISTINCT s.studentUserId) FROM SessionNote s " +
            "WHERE s.mentorUserId = :mentorUserId " +
            "AND s.progressStatus IN :statuses")
    long countStudentsByMentorAndStatusIn(
            @Param("mentorUserId") Long mentorUserId,
            @Param("statuses") Collection<ProgressStatus> statuses);

    /** Convenience helper for countAtRiskStudentsByMentor. */
    default long countAtRiskStudentsByMentor(Long mentorUserId) {
        return countStudentsByMentorAndStatusIn(
                mentorUserId,
                List.of(ProgressStatus.AT_RISK, ProgressStatus.CRITICAL)
        );
    }

    /** Latest note per student — used for summary dashboards. */
    @Query("SELECT s FROM SessionNote s " +
            "WHERE s.mentorUserId = :mentorUserId " +
            "AND s.createdAt = (" +
            "    SELECT MAX(s2.createdAt) " +
            "    FROM SessionNote s2 " +
            "    WHERE s2.studentUserId = s.studentUserId " +
            "      AND s2.mentorUserId = :mentorUserId" +
            ")")
    List<SessionNote> findLatestNotePerStudentForMentor(@Param("mentorUserId") Long mentorUserId);

    List<SessionNote> findByStudentUserIdAndProgressStatus(
            Long studentUserId, ProgressStatus status);
}